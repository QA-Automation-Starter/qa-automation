...
public class StreamMatchers {
    ...
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> yieldsNothing() {
        return new TypeSafeMatcher<S>() {
            ...
            @Override
            protected void describeMismatchSafely(S item,
                Description description) {
                description.appendText("the Stream started with ")
                    .appendValue(actualIterator.next());
                if (actualIterator.hasNext()) {
                    description
                        .appendText(" and will yield even more elements");
                } else {
    ...
    /**
     * A matcher for a finite Stream producing the same number of items as the
     * expected Stream, and producing equal items as expected in the same order.
     *
     ...
     * @param <T>
     *            The type of items produced by each BaseStream
     * @param <S>
     *            The type of BaseStream
     * @see #startsWith
     * @see #startsWithInt
     * @see #startsWithLong
     * @see #startsWithDouble
     */
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> yieldsSameAs(
        S expected) {
        return new BaseStreamMatcher<T, S>() {
            @Override
            protected boolean matchesSafely(S actual) {
                return remainingItemsEqual(expected.iterator(),
                    actual.iterator());
            }
        };
    }
     ...
     *             {@link org.hamcrest.Matchers#equalTo(Object)}, use
     *             {@link #yieldsSameAs(BaseStream)} instead
     */
    @Deprecated
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> equalTo(
        S expected) {
        return yieldsSameAs(expected);
    }

    /**
     * A matcher for potentially infinite Streams of objects where the first
     * limit items from each must be equal
     *
     * @param expected
     *            A Stream to check against
     * @param limit
     *            Only check this number of items from actual Stream
     * @param <T>
     *            The type of items produced by each Stream
     * @see #equalTo
     * @see #startsWithInt
     * @see #startsWithLong
     * @see #startsWithDouble
     */
    public static <T> Matcher<Stream<T>> startsWith(Stream<T> expected,
        long limit) {
        return new BaseStreamMatcher<T, Stream<T>>() {
            @Override
            protected boolean matchesSafely(Stream<T> actual) {
                return remainingItemsEqual(expected.limit(limit).iterator(),
                    actual.limit(limit).iterator());
            }
        };
    }

    /**
     * A matcher for potentially infinite Streams of primitive doubles where the
     * first limit items from each must be equal
     *
     * @param expected
     *            A Stream to check against
     * @param limit
     ...
     * @see #startsWithLong
     */
    public static Matcher<DoubleStream> startsWith(DoubleStream expected,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive ints where the
     * first limit items from each must be equal
     *
     * @param expected
     *            A Stream to check against
     ...
     * @see #startsWithDouble
     */
    public static Matcher<IntStream> startsWith(IntStream expected,
        long limit) {
     ...
     * @see #startsWithDouble
     */
    public static Matcher<LongStream> startsWith(LongStream expected,
        long limit) {
    ...
    private static void describeToStartsAllWith(Description description,
        long limit, Matcher<?> matcher) {
        description
            .appendText("First ")
    ...
    /**
     * A matcher for potentially infinite Streams of objects, the first limit of
     * which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     * @param limit
     *            Only check this number of items from the Stream
     * @see #allMatch
     * @see #startsWithAllLong
     * @see #startsWithAllInt
     * @see #startsWithAllDouble
     */
    public static <T> Matcher<Stream<T>> startsWithAll(Matcher<T> matcher,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive longs, the first
     * limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     * @param limit
     ...
     * @see #startsWithAllDouble
     */
    public static Matcher<LongStream> startsWithAllLong(Matcher<Long> matcher,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive ints, the first
     * limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     ...
     * @see #startsWithAllDouble
     */
    public static Matcher<IntStream> startsWithAllInt(Matcher<Integer> matcher,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive doubles, the
     * first limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     * @param limit
     ...
     * @see #startsWithAllLong
     */
    public static Matcher<DoubleStream> startsWithAllDouble(
        Matcher<Double> matcher, long limit) {
    ...
    private static void describeToStartsAnyWith(Description description,
        long limit, Matcher<?> matcher) {
        description
    ...
    /**
     * A matcher for potentially infinite Streams of objects, at least one of
     * the first limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     * @param limit
     ...
     */
    public static <T> Matcher<Stream<T>> startsWithAny(Matcher<T> matcher,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive longs, at least
     * one of the first limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     * @param limit
     ...
     * @see #startsWithAnyDouble
     */
    public static Matcher<LongStream> startsWithAnyLong(Matcher<Long> matcher,
        long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive doubles, at least
     * one of the first limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     ...
     * @see #startsWithAnyLong
     */
    public static Matcher<DoubleStream> startsWithAnyDouble(
        Matcher<Double> matcher, long limit) {
    ...
    /**
     * A matcher for potentially infinite Streams of primitive ints, at least
     * one of the first limit of which must match the given Matcher
     *
     * @param matcher
     *            A matcher to apply to items produced from the Stream
     ...
     * @see #startsWithAnyDouble
     */
    public static Matcher<IntStream> startsWithAnyInt(Matcher<Integer> matcher,
        long limit) {
    ...
    /**
     * The BaseStream must produce exactly the given expected items in order,
     * and no more.
     *
     * For infinite BaseStreams see {@link #startsWith(Object...)} or a
     * primitive stream variant
     *
     * @param expectedMatchers
     *            Matchers for the items that should be produced by the
     *            BaseStream
     * @param <T>
     *            The type of items
     * @param <S>
     ...
     */
    @SafeVarargs
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> yieldsExactly(
        Matcher<T>... expectedMatchers) {
        return new BaseMatcherStreamMatcher<T, S>() {
            @Override
            protected boolean matchesSafely(S actual) {
                return remainingItemsMatch(
                    new ArrayIterator<>(expectedMatchers), actual.iterator());
            }
        };
    }
     ...
     *             {@link #yieldsExactly(Matcher...)} instead
     */
    @SafeVarargs
    @Deprecated
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> contains(
        Matcher<T>... expectedMatchers) {
    ...
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> yieldsExactly(
        ...
        return new BaseStreamMatcher<T, S>() {
            @Override
            protected boolean matchesSafely(S actual) {
                return remainingItemsEqual(new ArrayIterator<>(expected),
                    actual.iterator());
            }
        };
    }
     ...
     *             {@link #yieldsExactly(Object...)} instead
     */
    @SafeVarargs
    @Deprecated
    public static <T, S extends BaseStream<T, ? extends S>> Matcher<S> contains(
    ...
    /**
     * A matcher for a finite Stream of objects, all of which must match the
     * given Matcher.
     *
     * For infinite Streams see {@link #startsWithAll}
     *
     * @param matcher
     *            A Matcher against which to compare items from the Stream
     * @param <T>
    ...
    /**
     * A matcher for a finite Stream of primitive ints, all of which must match
     * the given Matcher.
     *
    ...
    /**
     * A matcher for a finite Stream of primitive longs, all of which must match
     * the given Matcher.
     *
    ...
    /**
     * A matcher for a finite Stream of primitive doubles, all of which must
     ...
     * @see #allMatchLong
     */
    public static Matcher<DoubleStream> allMatchDouble(
    ...
    /**
     * A matcher for a finite Stream of objects, at least one of which must
     * match the given Matcher.
     *
    ...
    /**
     * A matcher for a finite Stream of primitive longs, at least one of which
    ...
    /**
     * A matcher for a finite Stream of primitive doubles, at least one of which
     * must match the given Matcher.
     *
     ...
     * @see #anyMatchDouble
     */
    public static Matcher<DoubleStream> anyMatchDouble(
        Matcher<Double> matcher) {
        return new DoubleStreamAnyMatches(matcher) {
            @Override
            public void describeTo(Description description) {
    ...
    /**
     * A matcher for a finite Stream of primitive ints, at least one of which
     * must match the given Matcher.
     *
    ...
    /**
     * A matcher for a potentially infinite Stream of objects against n expected
     * items, matching if the first n items produced by the Stream equal the
     * expected items in order. Whether the Stream would subsequently produce
     * additional items is irrelevant.
     *
     * @param expected
     *            The expected items produced first by the Stream
     * @param <T>
     *            The type of items
     * @see #contains
     * @see #startsWithInt
    ...
    public static <T> Matcher<Stream<T>> startsWith(T... expected) {
        return new BaseStreamMatcher<T, Stream<T>>() {
            ...
            protected boolean matchesSafely(Stream<T> actual) {
                return remainingItemsEqual(new ArrayIterator<>(expected),
                    actual.limit(expected.length).iterator());
            }
        };
    }

    /**
     * A matcher for a potentially infinite Stream of primitive doubles against
     * n expected items, matching if the first n items produced by the Stream
     * equal the expected items in order. Whether the Stream would subsequently
     * produce additional items is irrelevant.
     *
     * @param expected
     *            The expected items produced first by the Stream
     * @see #contains
    ...
    public static Matcher<DoubleStream> startsWithDouble(double... expected) {
        return new BaseStreamMatcher<Double, DoubleStream>() {
            @Override
            protected boolean matchesSafely(DoubleStream actual) {
                return remainingItemsEqual(new DoubleArrayIterator(expected),
                    actual.limit(expected.length).iterator());
            }
        };
    }
    ...
    /**
     * A matcher for a potentially infinite Stream of primitive longs against n
     * expected items, matching if the first n items produced by the Stream
     * equal the expected items in order. Whether the Stream would subsequently
     * produce additional items is irrelevant.
     *
     * @param expected
    ...
    public static Matcher<LongStream> startsWithLong(long... expected) {
        return new BaseStreamMatcher<Long, LongStream>() {
            @Override
            protected boolean matchesSafely(LongStream actual) {
                return remainingItemsEqual(new LongArrayIterator(expected),
                    actual.limit(expected.length).iterator());
            }
        };
    }
    ...
    /**
     * A matcher for a potentially infinite Stream of primitive ints against n
     * expected items, matching if the first n items produced by the Stream
     * equal the expected items in order. Whether the Stream would subsequently
     * produce additional items is irrelevant.
     *
    ...
    public static Matcher<IntStream> startsWithInt(int... expected) {
        return new BaseStreamMatcher<Integer, IntStream>() {
            @Override
            protected boolean matchesSafely(IntStream actual) {
                return remainingItemsEqual(new IntArrayIterator(expected),
                    actual.limit(expected.length).iterator());
            }
        };
    }
    ...
    private static abstract class BaseStreamMatcher<T, S extends BaseStream<T, ?>>
        ...
        private void describe(Description description, List<T> values) {
            description.appendText("Stream of ").appendValueList("[", ",", "]",
                values);
        }

        boolean remainingItemsEqual(Iterator<T> expectedIterator,
            ...
            if (expectedIterator.hasNext() && actualIterator.hasNext()) {
                ...
                if (Objects.equals(nextExpected, nextActual)) {
                    return remainingItemsEqual(expectedIterator,
                        actualIterator);
                }
            }
    ...
    private static abstract class BaseMatcherStreamMatcher<T, S extends BaseStream<T, ?>>
        extends TypeSafeMatcher<S> {
        ...
        boolean remainingItemsMatch(Iterator<Matcher<T>> expectedIterator,
            Iterator<T> actualIterator) {
            if (!expectedIterator.hasNext() && !actualIterator.hasNext()) {
                return true;
            }
            if (expectedIterator.hasNext() && actualIterator.hasNext()) {
                ...
                if (nextExpected.matches(nextActual)) {
                    return remainingItemsMatch(expectedIterator,
                        actualIterator);
                }
            }
            expectedIterator.forEachRemaining(expectedAccumulator::add);
    ...
    private static void allMatchMismatch(Description mismatchDescription,
        long position, Object nonMatch) {
        mismatchDescription.appendText("Item ")
            .appendText(Long.toString(position))
            .appendText(" failed to match: ").appendValue(nonMatch);
    }
    ...
    private static abstract class StreamAllMatches<T>
        ...
        @Override
        protected void describeMismatchSafely(Stream<T> actual,
            Description mismatchDescription) {
            allMatchMismatch(mismatchDescription, positionNonMatching,
                nonMatching);
        }
    }
    ...
    private static abstract class IntStreamAllMatches
        ...
        @Override
        protected void describeMismatchSafely(IntStream actual,
            Description mismatchDescription) {
            allMatchMismatch(mismatchDescription, positionNonMatching,
                nonMatching);
        }
    ...
    private static abstract class LongStreamAllMatches
        ...
        @Override
        protected void describeMismatchSafely(LongStream actual,
            Description mismatchDescription) {
            allMatchMismatch(mismatchDescription, positionNonMatching,
                nonMatching);
        }
    ...
    private static abstract class DoubleStreamAllMatches
        ...
        @Override
        protected void describeMismatchSafely(DoubleStream actual,
            Description mismatchDescription) {
            allMatchMismatch(mismatchDescription, positionNonMatching,
                nonMatching);
        }
    ...
    private static void anyMatchMismatch(Description mismatchDescription,
        List<?> accumulator) {
        mismatchDescription
