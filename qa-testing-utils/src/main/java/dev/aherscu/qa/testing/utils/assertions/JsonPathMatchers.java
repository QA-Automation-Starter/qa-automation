...
public class JsonPathMatchers {
    ...
    public static <T> Matcher<? super Object> hasJsonPath(String jsonPath,
        Matcher<T> resultMatcher) {
    ...
    public static Matcher<Object> isJson() {
        return isJson(withJsonPath("$",
            anyOf(instanceOf(Map.class), instanceOf(List.class))));
    }
    ...
    public static Matcher<String> isJsonString(
        Matcher<? super ReadContext> matcher) {
        return new IsJson<String>(matcher);
    }

    public static Matcher<File> isJsonFile(
        Matcher<? super ReadContext> matcher) {
        return new IsJson<File>(matcher);
    }

    public static Matcher<? super ReadContext> withJsonPath(String jsonPath,
        Predicate... filters) {
    ...
    public static Matcher<? super ReadContext> withoutJsonPath(String jsonPath,
        Predicate... filters) {
        return withoutJsonPath(JsonPath.compile(jsonPath, filters));
    }

    public static Matcher<? super ReadContext> withoutJsonPath(
        JsonPath jsonPath) {
        return new WithoutJsonPath(jsonPath);
    }

    public static <T> Matcher<? super ReadContext> withJsonPath(String jsonPath,
        Matcher<T> resultMatcher) {
        return withJsonPath(JsonPath.compile(jsonPath), resultMatcher);
    }

    public static <T> Matcher<? super ReadContext> withJsonPath(
        JsonPath jsonPath, Matcher<T> resultMatcher) {
