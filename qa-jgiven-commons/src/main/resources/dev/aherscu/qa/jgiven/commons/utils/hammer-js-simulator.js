...
(function () {
    ...
    Simulator.events = {
        ...
        pointer: {
            ...
            trigger: function (touches, element, type) {
                touches.forEach(function (touch, i) {
                    ...
                    const eventType = this.typeMap[type];
                    // ie10 style events
                    const msEventType = window.MSPointerEvent && eventType.replace(/pointer([a-z])/, function (_, a) {
        ...
        touch: {
            ...
            trigger: function (touches, element, type) {
                const touchList = this.emptyTouchList();
                touches.forEach(function (touch, i) {
                    const x = Math.round(touch.x),
                ...
                let targetTouches;
                if (type !== 'end') {
                    targetTouches = touchList.filter(function (touch) {
    ...
    function getTouches(center, elements, countTouches, radius, rotation) {
        ...
        const slice = 2 * Math.PI / countTouches;

        for (let i = 0; i < countTouches; i++) {
    ...
    function triggerGesture(elements, startTouches, options, done) {
        ...
        function gestureLoop() {
            ...
            const radius = options.scale !== 1
                ? options.radius - (options.radius * (1 - options.scale) * (1 / loops * loop))
                : options.radius;

            // calculate new position/rotation
            const easing = Simulator.easings[options.easing](1 / loops * loop),
    ...
    Simulator.gestures = {
        ...
        press: function (element, options, done) {
            ...
            const touches = getTouches(options.pos, element, 1);
        ...
        pan: function (element, options, done) {
            ...
            const touches = getTouches(options.pos, element, options.touches);
        ...
        pinch: function (elements, options, done) {
            ...
            const touches = getTouches(options.pos, elements, options.touches);
